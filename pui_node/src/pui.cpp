...
using city_science::pui::Pui;
using namespace std::chrono_literals;

Pui::Pui(const std::string & usb_port)
: Node("pui_node", rclcpp::NodeOptions().use_intra_process_comms(true))
{
  RCLCPP_INFO(get_logger(), "Init Pui Node Main");
...
Pui::Wheels * Pui::get_wheels()
{
  return &wheels_;
}

Pui::Motors * Pui::get_motors()
{
  return &motors_;
}
...
void Pui::publish_timer(const std::chrono::milliseconds timeout)
{
...
void Pui::heartbeat_timer(const std::chrono::milliseconds timeout)
{
...
void Pui::parameter_event_callback()
{
